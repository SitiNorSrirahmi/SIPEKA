<div class="bg-[#F0F4FB] min-h-screen">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {{-- ==================== SUCCESS ==================== --}}
        @if (session('success'))
        <div class="flex items-start gap-3 bg-emerald-50 border border-emerald-200 text-emerald-700 p-3 sm:p-4 mb-4 sm:mb-5 rounded-2xl">
        ...
        {{-- ==================== TOKEN ==================== --}}
        @if (session('token'))
        <div class="flex items-start gap-3 bg-blue-50 border border-blue-200 text-blue-800 p-3 sm:p-4 mb-4 sm:mb-5 rounded-2xl">
            <svg class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
            </svg>
            <div class="min-w-0">
                <p class="text-xs sm:text-sm font-semibold">Simpan token ini untuk cek status laporan Anda:</p>
                <p class="font-mono text-base sm:text-lg font-bold mt-1 tracking-wider break-all">{{ session('token') }}</p>
            </div>
        </div>
        @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div class="lg:col-span-2 space-y-4 sm:space-y-6">
                        ...
                        <div class="space-y-4 sm:space-y-5">
                            ...
                            <div>
                                ...
                                <input type="text" name="lokasi" id="input-lokasi"
                                    value="{{ old('lokasi') }}"
                                    placeholder="Klik peta / GPS, atau isi manual."
                                ...
                                <p class="text-[10px] sm:text-[11px] text-slate-400 mt-1.5">Klik peta atau "Gunakan Lokasi Saya" untuk isi otomatis.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-4 sm:p-6 lg:p-7">
                        ...
                        {{-- Pesan GPS status --}}
                        <div id="gps-status" class="hidden mb-3 sm:mb-4"></div>
                        ...
                        <div class="relative rounded-xl overflow-hidden border border-slate-200" style="position: relative; z-index: 1; isolation: isolate;">
                            <div id="peta-laporan" class="h-[280px] sm:h-[320px]" style="z-index: 0;"></div>

                            <button type="button"
                                id="btn-lokasi-saya"
                                onclick="gunakanLokasiSaya()"
                                class="absolute top-2 sm:top-3 right-2 sm:right-3 inline-flex items-center gap-1.5 sm:gap-2 bg-white hover:bg-blue-50 text-slate-800 border border-slate-300 hover:border-blue-400 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-lg text-[10px] sm:text-xs font-bold shadow-md transition-all duration-200 hover:-translate-y-0.5"
                                style="z-index: 400;">
                        ...
                        {{-- Field lat/long --}}
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-5 mt-3 sm:mt-5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-900 uppercase tracking-wider mb-2">
                        ...
                        <div class="space-y-4 sm:space-y-5">
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" rows="3"
                                    ...
                                    class="w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition text-sm text-slate-900 placeholder:text-slate-500 leading-relaxed resize-y">{{ old('deskripsi') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-4 sm:p-6 lg:p-7">
                        <div class="flex items-center gap-3 mb-4 sm:mb-6">
                            <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-indigo-50 flex items-center justify-center shrink-0">
                    ...
                    <div class="lg:sticky lg:top-6 space-y-4 sm:space-y-6">
                        ...
                        <div class="bg-white rounded-2xl border border-slate-200 p-4 sm:p-6">
                            <div class="flex items-center gap-3 mb-4 sm:mb-5">
                                <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-amber-50 flex items-center justify-center shrink-0">
                            ...
                            <div id="preview-container" class="rounded-xl overflow-hidden bg-slate-100 aspect-video mb-3 sm:mb-4">
                                <div id="preview-empty" class="w-full h-full flex flex-col items-center justify-center">
                                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-slate-200 flex items-center justify-center mb-2">
                                        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                                                d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                        </svg>
                                    </div>
                                    <p class="text-[10px] sm:text-xs text-slate-400">Belum ada foto</p>
                                </div>
                            ...
                            <input type="file" name="foto" accept="image/*"
                                ...
                                class="w-full text-[10px] sm:text-xs text-slate-500 file:mr-2 sm:file:mr-3 file:py-2 sm:file:py-2.5 file:px-3 sm:file:px-4 file:rounded-lg file:border-0 file:text-[10px] sm:file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 file:cursor-pointer cursor-pointer border border-slate-200 rounded-xl bg-slate-50/50 p-1.5 transition">
                            <p class="text-[10px] sm:text-[11px] text-slate-400 mt-2">Format: JPG, PNG. Maks 2MB.</p>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function reverseGeocode(lat, lng) {
            ...
            try {
                ...
                inputLokasi.placeholder = 'Klik peta / GPS, atau isi manual.';
        ...
        function showGpsStatus(type, message) {
            ...
            const baseClass = 'flex items-start gap-2 p-2.5 sm:p-3 rounded-xl text-[11px] sm:text-xs border ';
            const styleMap = {
